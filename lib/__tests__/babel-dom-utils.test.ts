...
describe('Babel Utilities', () => {
  it('should return an index array describing path to DOM element', () => {
    const result = getPathIndexes(p1, rootDiv);
    expect(result).toEqual(pathIndexes);
  });
  ...
  it('should return selected babel node from index array', () => {
    ...
    const selectedNode = getSelectedNode(rootNode, pathIndexes);
  ...
  it('should return DOM element from index array', () => {
    const selectedElement = getSelectedElement(rootDiv, pathIndexes);
